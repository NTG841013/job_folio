...
export default function ResearchChart({ data }: ResearchChartProps) {
  return (
    <div className="bg-surface border border-border rounded-2xl p-6 shadow-sm h-full flex flex-col">
      <h3 className="text-lg font-bold text-text-primary mb-6">Company Research Activity</h3>
      <div className="flex-1 min-h-[250px] w-full relative">
        {data.length === 0 && (
          <div className="absolute inset-0 flex items-center justify-center bg-surface/50 z-10 rounded-xl">
            <p className="text-sm text-text-muted">No research activity yet</p>
          </div>
        )}
